...
export const db = createClient({
  url: process.env.CI
    ? // biome-ignore lint/style/noNonNullAssertion: It's fucking there.
      process.env.TURSO_DATABASE_URL!
    : "file:../data/otdb.sqlite",
});
...
`;
  ...
  ).rows as unknown as SymbolSearchResult[];
};
...
`;
  ...
  return transformExpressionResults(
    (
      await db.execute({
        sql,
        args: [...transcriptIds, dataset],
      })
    ).rows as unknown as BaseExpressionSearchResult[]
...
ORDER BY
  ...
  return (await db.execute({ sql, args: [transcriptId] }))
    .rows as unknown as GeneAnnotation[];
};
...
ORDER BY
  ...
  ).rows as unknown as MRNAAnnotation[];
};
...
ORDER BY
  ...
  return transformGoTerms(
    ...
    ).rows as unknown as BaseGoTermsAnnotation[]
...
ORDER BY
  ...
  ).rows as unknown as PathwayAnnotation[];
};
...
ORDER BY
  ...
  ).rows as unknown as ProteinDomainAnnotation[];
};
...
;
  ...
  ).rows as unknown as SwissprotAnnotation[];
};
...
ORDER BY
  ...
  return transformUnigene(
    ...
    ).rows as unknown as BaseUnigeneAnnotation[]
...
ORDER BY
   ...
   * )
   */
  const res = transformExpressionByTissue(
    (
      await db.execute({
        sql,
        args: [...transcriptIds, dataset],
      })
    ).rows as unknown as BaseExpressionByTissue[]
